  ...
  return blocks.map((block, i) => {
    ...
    if (trimmed.startsWith("### ")) {
      return (
        <h3
          key={i}
          className="text-white text-heading-h4 font-medium mt-10 mb-4"
        >
          {trimmed.replace("### ", "")}
        </h3>
      );
    ...
    if (trimmed.startsWith("## ")) {
      return (
        <h2
          key={i}
          className="text-white text-heading-h3 font-medium mt-12 mb-5"
        >
          {trimmed.replace("## ", "")}
        </h2>
      );
    ...
    return (
      <p
        key={i}
        className="text-gray-300 text-body-base leading-relaxed mb-6"
      >
        {trimmed}
      </p>
    );
    ...
    <article className="w-full bg-[#040A0C] min-h-screen">
      <div className="w-full max-w-[820px] mx-auto px-4 md:px-0 py-12 md:py-16">
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {post.tags.map((tag) => (
              <span
                key={tag}
                className="bg-white/10 border border-white/20 text-white text-xs px-4 py-1.5 rounded-full"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="flex items-center gap-4 pb-8 mb-10 border-b border-white/10">
          <div className="w-12 h-12 rounded-full bg-water-700 overflow-hidden flex-shrink-0">
            <Image
          ...
          <div>
            <p className="text-white text-body-base font-medium">
              {post.author.name}
            </p>
            <p className="text-gray-400 text-body-small">
              {post.author.role} &middot; {post.date} &middot; {post.readTime}
            </p>
          </div>
        </div>
